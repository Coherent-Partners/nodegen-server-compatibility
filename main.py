...
def main():
    ...
    if len(features_pd) > 0:
        ...
        json.dump({"features": features, "runner_versions": runner_versions}, open("nodegen-server-compatibility.json", "w", encoding='utf-8'), separators=(',', ':'))
    # Only output the runner_versions to JSON file
    else:
        json.dump({"runner_versions": runner_versions}, open("nodegen-server-compatibility.json", "w", encoding='utf-8'), separators=(',', ':'))

    ############################
    # Markdown for human reading
    ############################

    md_file = MdUtils(file_name="README", title="Coherent Spark Runner Compiler Compatibility")

    md_file.new_paragraph("The Spark team developed [`nodegen-server`](https://github.com/orgs/Coherent-Partners/packages/container/package/nodegen-server) (also referred to as the Neuron runner) to create and run containerized applications designed explicitly for executing [Wasm](https://webassembly.org/) modules generated by Spark. The release history of [`nodegen-server`](https://github.com/orgs/Coherent-Partners/packages/container/package/nodegen-server) can be found in [Hybrid Runner Release History](https://docs.coherent.global/hybrid-runner/hybrid-runner-release-history).")
    md_file.new_paragraph("This page details the compatibility between a version of the [`nodegen-server`](https://github.com/orgs/Coherent-Partners/packages/container/package/nodegen-server) against the Spark [Neuron compiler](https://docs.coherent.global/build-spark-services/neuron/release-history). The compiler version can be found from the Spark UI under the _[Service Documentation Version details](https://docs.coherent.global/navigation/service-documentation#version-details)_ or in the [_Execute API_ `response_meta`](https://docs.coherent.global/spark-apis/execute-api/execute-api-v3#response_meta).")
    # md_file.new_paragraph("The runner includes configuration parameters to perform checks and whether or not to continue execution if a version of Neuron is not fully supported.")
    md_file.new_paragraph("[`nodegen-server`](https://github.com/orgs/Coherent-Partners/packages/container/package/nodegen-server) `>=1.31.0` includes the capability and configuration for processing compatibility versions and the version compatibility mapping. Compiled [Wasm](https://webassembly.org/) modules from `Neuron` `>=1.22.1` embed the Neuron version that is also needed for the compatibility check.")
    md_file.new_paragraph("The releases include the download for a JSON file mapping the compatibility versions.")
    md_file.new_paragraph("")

    if len(features_pd) > 0:
        md_file.new_header(level=1, title="Compatibility features")
        md_file.new_paragraph("List of features that require specific runner version(s).")
        md_file.new_table(columns=features_pd.shape[1], rows=len(features_pd)+1, text=list(features_pd) + [item for sublist in features_pd.astype(str).values.tolist() for item in sublist], text_align='center')

    md_file.new_header(level=1, title="Compatibility versions")
    md_file.new_paragraph("List of all released runner versions and their corresponding __highest__ supported Neuron version.")
    md_file.new_table(columns=runner_versions_pd.shape[1], rows=len(runner_versions_pd)+1, text=list(runner_versions_pd) + [item for sublist in runner_versions_pd.astype(str).values.tolist() for item in sublist], text_align='center')
