...
# file locations
# if run in script mode use the __file__ path to define location
path = Path(globals().get("__file__", "./_")).absolute().parent
...
runner_versions_pd = pd.read_csv(os.path.join(path, "nodegen-server Neuron versions.csv"))
...
# features_pd = pd.read_csv(os.path.join(path, "nodegen-server Features (Testing).csv")).sort_values(by="feature")
features_pd = pd.read_csv(os.path.join(path, "nodegen-server Features.csv")).sort_values(by="feature")
...
if len(features_pd) > 0:
    ...
    json.dump({"features": features, "runner_versions": runner_versions}, open(os.path.join(path, "nodegen-server-compatibility.json"), "w"), separators=(',', ':'))
# Only output the runner_versions to JSON file
else:
    json.dump({"runner_versions": runner_versions}, open(os.path.join(path, "nodegen-server-compatibility.json"), "w"), separators=(',', ':'))

############################
# Markdown for human reading
############################

mdFile = MdUtils(file_name=os.path.join(path, "README"), title="Coherent Spark Runner Compiler Compatibility")
